...
// chat_log の取得
$sql = "SELECT * FROM chat_log WHERE page_uid = :page_uid ORDER BY created_at DESC LIMIT 50 ";
    ...
    <div class="chat" @click=" activeIndex = (activeIndex === chat.chat_id) ? null : chat.chat_id ">
      ...
      <div class="content" v-if="activeIndex === chat.chat_id">
        ...
        <div
          ...
          :key="index"
          :class="['chat-row', 'chant_contents_wrap', cnv.role == 'user' ? 'user' : 'bot']"
        >
          <div class="bubble">{{ cnv.content }}</div>
        </div>
      </div>
  </div>
</main>
