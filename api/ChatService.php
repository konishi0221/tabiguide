...
/* ---------- ChatService.php ---------- */
    ...
    {
        // ChatService.php が api/ にある想定:
        //   /workspace/api/ChatService.php
        //   /workspace/api/prompts/chat_system.txt
        $path = __DIR__.'/prompts/chat_system.txt';
    ...
    private function searchFAQ(string $kw): ?array
    {
        ...
        $sql = 'SELECT id,question,answer,tags,hits FROM question
                ...
                ORDER BY hits DESC, pinned DESC, updated_at DESC LIMIT 1';
        $st = $this->db()->prepare($sql);
        $st->execute([':uid'=>$this->pageUid, ':nat'=>$kw]);
        $row = $st->fetch(PDO::FETCH_ASSOC) ?: null;
        if ($row) {
            $this->db()->prepare('UPDATE question SET hits=hits+1 WHERE id=?')
                       ->execute([$row['id']]);
        }
        return $row;
    }
    ...
    {
        return $this->db ??= require dirname(__DIR__).'/public/core/db.php';
    }
